...
class Upsample_BLC(nn.Module):
    ...
    def __init__(self, in_feat, type='pixelshuffle'):
        super(Upsample_BLC, self).__init__()
...
class Task_Attention(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        q = self.q(task_q).reshape(B, task_q.shape[1], self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)  # (B,num_heads,task_q_num,head_dim)

        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1).reshape(B, C, H, W)
            x_ = self.sr(x_).reshape(B, C, -1).permute(0, 2, 1)
            x_ = self.norm(x_)
            kv = self.kv(x_).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)  # (kv,B,num_heads,hw,head_dim)
        else:
            kv = self.kv(x).reshape(B, -1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
...
class Query_Attention(nn.Module):
    ...
    def forward(self, task_q, x, H=None, W=None):
        ...
        x = (attn @ v)
        x = rearrange(x, 'b head n c -> b n (head c)')
        x = self.proj(x)
        x = self.proj_drop(x)

        return x

class Query_Self_Attention(nn.Module):
    """
    x: B,hw,C; B,HW,C
    return: B,hw,C
    process: mlp_q(task_query) + mlp_kv(optional sr) + HW x hw attention(scale)(drop_path) -> value + mlp(drop)
...
class Standard_TransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, task=False):
        super().__init__()
        ...
        self.mlp = Standard_Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class Self_TransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Query_Self_Attention(
            dim,
            num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
            attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)
